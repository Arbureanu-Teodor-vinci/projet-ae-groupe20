    ...
    try {
      ...
      PreparedStatement ps = dalConn.getPS(
          "SELECT c.id_contacts, c.interview_method, c.tool, c.refusal_reason, c.state_contact,\n"
              ...
              + "       e.id_enterprise, e.trade_name, e.designation, e.phone_number as enterprise_phoneNumber,\n"
              + "       e.address, e.city,e.email as enterprise_email, e.black_listed, e.black_listed_motivation,\n"
              + "       e.version as enterprise_version\n"
              + "FROM InternshipManagement.contacts c\n"
              + "JOIN InternshipManagement.academic_year a1 ON c.academic_year = a1.id_academic_year\n"
              ...
              + "JOIN InternshipManagement.academic_year a2 ON s.academic_year = a2.id_academic_year\n"
              + "WHERE c.id_contacts = ?"
      );
      ps.setInt(1, id);
