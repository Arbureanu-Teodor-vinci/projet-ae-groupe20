...
async function renderUpdateContactPage() {
  ...
  if (!response.ok) {
    ...
    const responseEnterprise = await fetch(
      `/api/enterprises/getOne:${contact.enterpriseId}`,
      options,
    );
                ...
                <div class="col-12 text-center">
                    ...
                    <form id="contactForm">
                        <div class="form-group">
                            ...
                            <input type="text" id="enterpriseId" name="enterpriseId" class="form-control text-center" value="${
                        ...
                        <div class="form-group interviewMethodDiv">
                            ...
                            <select id="interViewMethod" name="interViewMethod" class="form-control text-center" ${nonUpdatable}>
                                <option value="A distance" ${
                                  contact.interViewMethod === 'A distance' ? 'selected' : ''
                                }>A distance</option>
                                <option value="Dans l entreprise" ${
                                  contact.interViewMethod === 'Dans l entreprise' ? 'selected' : ''
                                }>Dans l'entreprise</option>
                            </select>
                        </div>
                        <div class="form-group tool">
                            <label for="tool"><B>Outil de contact</B></label>
                            <input type="text" id="toolInput" name="tool" class="form-control text-center" value="${
                              contact.tool === null ? '' : contact.tool
                            }" ${nonUpdatable}>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group refusalReason">
                            ...
                            <input type="text" id="refusalReasonInput" name="refusalReason" class="form-control text-center" value="${
                              contact.refusalReason === null ? '' : contact.refusalReason
                            }" ${nonUpdatable}>
                        </div>
                        ${
                          nonUpdatable
    ...
    interViewMethodSelectDiv.style.display =
      stateContactSelect.value === 'initié' || stateContactSelect.value === 'non suivis'
        ? 'none'
        : 'block';
    ...
    stateContactSelect.addEventListener('change', (event) => {
      ...
      interViewMethodSelectDiv.style.display =
        event.target.value === 'initié' || event.target.value === 'non suivis' ? 'none' : 'block';
      toolDiv.style.display =
        event.target.value === 'initié' ||
        event.target.value === 'non suivis' ||
        interViewMethodSelect.value === 'Dans l entreprise'
          ? 'none'
          : 'block';
    });
    ...
    if (nonUpdatable === '') {
      document.getElementById('updateContactButton').addEventListener('click', async (event) => {
        ...
        const interviewMethod =
          interViewMethodSelect.value === '' || interViewMethodSelect === 'null'
            ? null
            : interViewMethodSelect.value;
        const tool = toolInput.value === '' || toolInput.value === 'null' ? null : toolInput.value;
        const stateContact = stateContactSelect.value;
        const refusalReason =
          refusalReasonInput.value === '' || refusalReasonInput.value === 'null'
            ? null
